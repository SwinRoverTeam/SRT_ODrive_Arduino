...
#include <Arduino.h> // millis(), Serial, etc.

// ---- protocol bits (unchanged except formatting) ----
union float_conv
{
  float flt;
  uint32_t u32;
...
{
private:
  uint8_t _node_id; // caution: 0x3f is broadcast ID
  int (*can_send_msg)(uint16_t id, uint8_t len, uint8_t *data, bool rtr);
  uint32_t _mtr_last_hb = 0;
  float_conv flt_cnv{};
