...
public:
    ...
    static void handleSerialCommand(const String &cmd, SRT_CanOpenMtr* motors, size_t num_Openmotors, 
                                   const uint8_t* node_ids, uint32_t accel_ms, uint32_t decel_ms);
    static String readSerialLine();

    //=========== Homing attempts =========
    int do_homing(); // run homing sequence, set current pos = 0
    int move_absolute(int32_t target, uint32_t accel_ms = 1000, uint32_t decel_ms = 1000);
};
